...
class ENet(nn.Module):
    ...
    def forward(self, x, all_features=False):
        ...
        # Stage 4 - Decoder
        x = self.upsample4_0(x, max_indices2_0, output_size=torch.Size([1, 32, 160, 160]))
        x = self.regular4_1(x)
        x = self.regular4_2(x)
        f5 = x
        ...
        # output
        x = self.final(x)
        x = self.sigmouid(x).permute(0, 2, 3, 1)

        if all_features:
            return [f1, f2, f3, f4, f5, f6], x
        return x
